...
class NotifyFetcher:
    ...
    async def get_all_notify_lists(self, time_a, time_b):
        ...
        while True:
            ...
            if hasNext is False:
                live_list = await Process_Notify(all_contents)._extract_media_items(time_a, time_b)
                return live_list
    ...
    async def basic_sort_josn(self):
        if self.json_data.get('code') == '0000':
            ...
            params = await self.build_params(cursor)
            return contents, params, hasNext
        
    async def build_params(self, cursor: Optional[str]) -> Dict[str, Any]:
        params = {"pageSize": 100, "languageCode": "en"}
...
class NCA005(Notification):
    ...
    def _parse_additional_info(self):
        info = self.additionalInfo.get("notificationInfo", {})
        return (
...
class Process_Notify:
    ...
    async def match(self):
        async for note in self.unpack_contents():
            handler = HANDLERS.get(note.notificationCase, Other_NCA)
    ...
    async def _extract_media_items(
        self,
        time_a: Optional[datetime] = None, time_b: Optional[datetime] = None) -> Tuple[List[Dict], List[Dict]]:
        live_list = []
        # 檢查是否需要進行時間篩選 -> bool
        ...
        async for h in self.match():
            if isinstance(h, (NCA005, NCA011)):
                media = {'mediaId':h.media_id, 'mediaType':'LIVE', 'title':h.context, 'thumbnailUrl':h.imageUrl, 'publishedAt':h.publishedAt, 'communityId':h.communityId, 'isFanclubOnly':h.publishedAt, 'live':h.liveStatus}
                published_at_str = h.publishedAt
                # 進行時間範圍篩選
