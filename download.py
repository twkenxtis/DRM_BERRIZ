...
class MediaDownloader:
    ...
    def _create_output_dir(self) -> Path:
        output_dir = Path(f"{self.media_id} {datetime.now().strftime('%y%m%d_%H%M')}")
...
async def run_dl(mpd_uri, decryption_key, json_data):
    ...
    if success:
        logging.info(f"\nDownload complete! File saved to: {downloader.base_dir}")
        logging.info(f"Video file: {downloader.base_dir / 'video.ts'}")
        logging.info(f"Audio file: {downloader.base_dir / 'audio.ts'}")
        
        # Mux video and audio with FFmpeg
        muxer = FFmpegMuxer(downloader.base_dir, decryption_key)
        if muxer.mux_to_mp4():
            logging.info(f"Final output file: {downloader.base_dir / 'output.mp4'}")
            t = json_data.get("media", {}).get("formatted_published_at", "")[2:-6].replace('-', '')
            video_codec = VideoInfo(downloader.base_dir / 'output.mp4').codec
            video_quality_label = VideoInfo(downloader.base_dir / 'output.mp4').quality_label
            video_audio_codec = VideoInfo(downloader.base_dir / 'output.mp4').audio_codec
            filename = f'{t} IVE - ' + json_data.get("media", {}).get("title") + f" WEB-DL.{video_quality_label}.{video_codec}.{video_audio_codec}.mp4"
            os.rename(downloader.base_dir / 'output.mp4', downloader.base_dir / filename)
            logging.info(f"Final output file: {downloader.base_dir / filename}")
