...
async def community_join(communityId:int):
    communityId, communityname = await community_id_name(communityId)
    try:
        if communityname is None:
            raise ValueError(f'{communityname} should be not none.')
        while True:
            logger.info(f"{Color.fg('light_gray')}try join to {Color.fg('aquamarine')}{communityname}{Color.reset()}")
            name = str(input(f"{Color.fg('light_yellow')}Please enter name for your {Color.fg('aquamarine')}[{communityname}]{Color.fg('light_yellow')} community's nickname:{Color.reset()} ").strip())
            if len(name) > 15:
                logger.warning(f'{name} community name only accept length < 15')
            else:
                break
        data = await Community().create_community(communityId, name)
        if data['code'] == '0000':
...
async def leave_community_main(communityId:int):
    communityId, communityname = await community_id_name(communityId)
    try:
        if communityname is None:
            raise ValueError(f'{communityname} should be not none.')
        while True:
            ...
            userinput = str(input(f'{Color.fg("gold")}typing YES to accept: {Color.reset()}')).strip()
            if userinput == 'YES':
                break
            elif userinput == 'yes':
                logger.warning('Try typing in all caps')
        data = await Community().leave_community(communityId)
        if data['code'] == '0000':
            logger.info(
            ...
            if data['code'] == 'FS_CM1010':
                logger.info(f"{Color.fg('light_gray')}{data['message']}{Color.reset()}")
...
async def community_id_name(communityId):
...
def print_data_with_fstring(data: dict):
