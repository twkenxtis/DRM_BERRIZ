...
async def berriz_verification_email_code(email: str):
    ...
    if str(response.status_code) == '201':
        if response.json()['code'] == '0000':
            ...
            logger.warning(f"{Color.fg('golden')}{response.json()['message']}{Color.reset()}")
            return False
    else:
...
async def post_verification_key(email: str, otpInt: str):
    ...
    if response.json()['code'] == '0000':
        ...
        logger.warning(f"{Color.fg('golden')}{response.json()['message']} resend a new code for {Color.reset()}{email}")
...
async def member_unlock(email: str, verifiedKey: str):
    ...
    if response.json()['code'] == '0000':
        logger.info(f"{Color.fg('green')}{response.json()['message']}{Color.reset()}")
        return True
    elif response.json()['code'] == 'FS_ER5010':
      logger.warning(f"{response.json()['message']} This account may not be registered in the Berriz database")
      return False
    if response.json()['code'] != '0000':
        logger.error(f"{Color.fg('golden')}{response.json()['message']}{Color.reset()}")
        return False
