...
async def vaild_email(email: str):
    params = {
        'email': email,
        'languageCode': 'en',
    }
    url = 'https://account.berriz.in/member/v1/members:signup-email-exists'
    response = await R.get(url, params)
    if response.json()['code'] == '0000':
        return response.json()['data']['exists'] # 預期是False
    elif response.json()['code'] != '0000':
        if response.json()['code'] == 'FS_ME1010':
            print(response.json()['message'], '->', email)
        else:
            logging.error(response.json())
            raise Exception(response.json())
    
async def setp2(email: str):
    params = {
        'languageCode': 'en',
    }
    json_data = {
...
async def post_verification_key(email: str, otpInt: str):
    ...
    if response.json()['code'] == '0000' and response.json()['message'] == 'OK':
        ...
        await SignupMannger.sign_up(email)
    elif response.json()['code'] != '0000':
        logging.error(response.json())
        return None
...
class SignupMannger:
    def __init__(self, email, password, clientid):
    ...
    async def sign_up(self):
        if self.validate_password_regex() is False:
        ...
        elif self.validate_password_regex() is True:
            # PKCE 請求
            ...
            if await vaild_email(self.account) is False:
                if await setp2(self.account) is True:
                    otpInt = self.handle_user_input()
                    verifiedKey = await post_verification_key(self.account, otpInt)
                    await terms(email, password, verifiedKey)
                    authkey = await authorizeKey(codeChallenge, state, self.CLIENTID)
